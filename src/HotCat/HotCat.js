...
var conf = $.extend({}, mw.config.values, {
	...
	wgServer: window.location.host.indexOf('.m.') > -1 ? '//' + window.location.host : mw.config.get('wgServer')
});
...
function loadURI(href, callback) {
	var url = href;
	if (url.slice(0, 2) === '//') url = window.location.protocol + url; else if (url.slice(0, 1) === '/') url = conf.wgServer + url;
	load(url, callback);
}
...
if (conf.wgUserLanguage !== 'en') {
	...
	if (window.hotcat_translations_from_commons && conf.wgServer.indexOf('//commons') < 0) {
		loadURI('/index.php?title=' + 'MediaWiki:Gadget-HotCat-' + conf.wgUserLanguage + '.js&action=raw&ctype=text/javascript', loadTrigger.loaded);
	} else {
		// Load translations locally
		loadJS('MediaWiki:Gadget-HotCat-' + conf.wgUserLanguage + '.js', loadTrigger.loaded);
	}
} else {
	loadTrigger.loaded();
}
...
function change_category(wikitext, toRemove, toAdd, key, is_hidden) {
	...
	if (toRemove && toRemove.length) {
		...
		if (i >= 0) before = before.slice(0, Math.max(0, i + 1)); else before = '';
		if (j < after.length) after = after.slice(Math.max(0, j)); else after = '';
...
function initialize() {
	...
	HC.no_autocommit = window.hotcat_no_autocommit !== undefined ? !!window.hotcat_no_autocommit : config.HotCatNoAutoCommit !== undefined ? config.HotCatNoAutoCommit :
		...
		var isMinorChange = function isMinorChange() {
			...
			var except = function except(aArr, bArr) {
				...
				for (var i = 0; i < lArr.length; i++) {
					...
					if (ind === -1) result.push(item); else sArr.splice(ind, 1); // don't check this item again
				}
			...
			if (sum.value && sumA.value === HC.changeTag) {
				...
				var removeChangeTag = function removeChangeTag() {
					$(eForm.wpTextbox1).add(sum).one('input', function () {
						window.setTimeout(function () {
							if (!isMinorChange()) $ct.val(''); else removeChangeTag();
						}, 500);
					});
				};
				removeChangeTag();
