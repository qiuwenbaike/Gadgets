/**
 * SPDX-License-Identifier: CC-BY-SA-4.0
 * _addText: '{{Gadget Header|license=CC-BY-SA-4.0}}'
 *
 * @url https://www.qiuwenbaike.cn/wiki/MediaWiki:Gadget-Wikiplus.js
 * @license <https://creativecommons.org/licenses/by-sa/4.0/>
 */
'use strict';

if (mw.config.get('wgAction') === 'view' && mw.config.get('wgIsArticle')) {
	mw.loader.load('/index.php?title=MediaWiki:Gadget-Wikiplus.js&action=raw&ctype=text/javascript&smaxage=3600&maxage=3600');
	if (!('ontouchstart' in document)) {
		mw.loader.addStyleTag('.Wikiplus-Symbol-Btn{vertical-align:bottom!important}.mw-ui-icon-portletlink-wphl-settings:before{background-image:url(data:image/svg+xml;base64,)}');
		mw.loader.load('/index.php?title=MediaWiki:Gadget-Wikiplus-highlight.js&action=raw&ctype=text/javascript&smaxage=3600&maxage=3600');
	}
}
