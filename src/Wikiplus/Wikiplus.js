 ...
 * _addText: '{{Gadget Header|licnese=CC-BY-SA-3.0}}'
 *
 * @source https://wikiplus-app.com/Main.min.js
...
$(function () {
	...
	var Wikipage = (function () {
		...
		_createClass(Wikipage, [ {
			...
			key: 'init',
			value: function init(title, callback, _config) {
				if (title === undefined) {
					title = this.pageName;
				}
		...
		var Wikiplus = (function () {
			_createClass(Wikiplus, [ {
				...
				value: function initQuickEdit() {
					...
					// 顶部编辑入口
					var topBtn = $('<li>').attr('id', 'Wikiplus-Edit-TopBtn').append($('<span>').append($('<a>').attr('href', 'javascript:void(0)').text(String(i18n('quickedit_topbtn'))))).data({
						number: -1,
					...
					if ($('.mw-editsection').length > 0) {
						...
						$('.mw-editsection').each(function (_i) {
							try {
				...
				value: function editEveryWhere() {
					var self = this;
					$('#mw-content-text a.external').each(function (_i) {
				...
				key: 'checkInstall',
				value: function checkInstall(_callback) {
					var self = this;
