 ...
 *
 * @url https://www.qiuwenbaike.cn/wiki/MediaWiki:Gadget-Popups-main.js
...
$(function () {
	...
	Insta.convert = function (wiki) {
		...
		function parse_inline_images(str) {
			...
			while ((start = str.indexOf('[[', substart)) !== -1) {
				if (str.slice(start + 2).match(RegExp('^(Image|File|' + Insta.conf.locale.image + '):', 'i'))) {
					loop = true;
					substart = start;
					do {
						substart += 2;
  ...
  /* eslint-enable */
	...
	function doSelectionPopup() {
		...
		if (close > 0 && sel.slice(Math.max(0, close + 2)).indexOf('[[') >= 0) {
			return;
		}
		var a = document.createElement('a');
    ...
    */
	...
	function setTitleBase() {
		...
		pg.re.basenames = RegExp('^(' + map(literalizeRegex, [ pg.wiki.titlebase,
			// pg.wiki.titlebase2,
	...
	navlinkTag.prototype.getPrintFunction = function () {
		...
		switch (this.id) {
			...
			default:
				this.print = function () {
					return String('Unknown navlink type: ' + this.id);
				};
		}
	};
	//
